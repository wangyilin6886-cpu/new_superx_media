...
| AI 教育 | Veo 3.1（同步对白，默认）· Kling 3.0 · Seedance 2.5 · HappyHorse 1.0 |
...
**四个模式全部实现**：AI 短视频、AI 短剧、AI 教育、AI App（Vibe Coding）。
...
### AI 教育 · studio.html?mode=education

打开 → 点「只给一个知识点」→ 发送 → 等大纲出来 → 点「出整节课」。会看到：

- 解析教材、编排课件讲稿两步，展开能看到知识点数量与课件结构
- 知识点图谱产物卡（8 个点，3 个核心点高亮）
- **三样产物依次生成**：课件 24 页 → 讲课视频 08:40 → 随堂练习 3 题
- 右栏「课件」：翻页器 + 6 页代表页 + 缩略图条 + 导出 PPTX/PDF
- 右栏「讲课视频」：**讲师形象与课件分屏**，点播放讲师口型会动、
  进度走到哪一段右边课件就翻到哪一页；下方可换讲师（换讲师只重渲染视频，
  课件和练习不动）或克隆自己
- 右栏「练习题」：3 题，正确项高亮，可展开逐题解析
- 右栏「设置」：学段 / 课时 / 难度 / 板书风格 / 导出格式 + LMS 对接说明

三样产物同源是这条线的核心：课件、讲稿、练习都由同一份知识点图谱生成，
改课件里的知识点，讲稿和练习会跟着重写。
...
左栏的项目与会话**每个模式一套**（短视频是投放批次、短剧是剧集系列、
教育是学科单元、AI App 是工具项目），当前会话名与顶栏标题一致。
数据在 `studio.js` 里各模式的 `nav: { folders, recent }`。

侧栏折叠后，展开入口在顶栏左上角的 ☰ —— 折叠时 `.nav` 宽度为 0
且 `overflow:hidden`，里面的折叠按钮点不到，所以展开按钮必须在外面。
